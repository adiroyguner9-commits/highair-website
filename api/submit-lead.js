...
export default async function handler(req, res) {
  ...
  console.log('[submit-lead] Received lead:', fields['Name'], '|', fields['Expedition']);
  ...
  console.log('[submit-lead] RESEND_API_KEY present:', !!RESEND_KEY);
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(109,40,217,0.10);max-width:600px;width:100%;">
        ...
        <tr><td style="background:linear-gradient(135deg,#4338ca,#7c3aed);padding:28px 32px;text-align:center;">
          <h1 style="margin:0;color:#fff;font-size:22px;font-weight:700;">🏔️ ליד חדש מהאתר</h1>
        ...
        <tr><td style="padding:32px;">
          ...
          <div style="background:#F5F0FF;border-right:4px solid #7c3aed;border-radius:8px;padding:16px 20px;margin-bottom:20px;">
            ...
            <p style="margin:6px 0 0;font-size:18px;font-weight:700;color:#1e1b4b;">${fields['Expedition']}</p>
          </div>` : ''}

          <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #ECEAF8;border-radius:12px;overflow:hidden;">
            ${row('👤 שם מלא',      fields['Name'])}
            ${row('📱 טלפון',       fields['Phone'])}
            ${row('📧 מייל',        fields['Email'])}
            ${row('📅 חודש מועדף', fields['Preferred Month'])}
            ${row('🎂 גיל',         fields['Age'])}
            ${row('👥 גודל קבוצה', fields['Group Size'])}
            ${row('🏔️ ניסיון',     fields['Experience'])}
            ${row('📍 מקור',        fields['Source'])}
          </table>
        </td></tr>
    ...
    try {
      const emailRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `🏔️ ליד חדש: ${fields['Name'] || 'לא צוין'} — ${fields['Expedition'] || 'לא צוין'}`,
          html:    emailHtml,
        }),
      });
