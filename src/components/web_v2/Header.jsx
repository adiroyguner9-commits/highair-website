...
export default function Header() {
  ...
  return (
    <header style={{
      ...
      display:             'grid',
      gridTemplateColumns: isMobile ? 'auto 1fr' : '1fr auto',
      alignItems:          'center',
      direction:           'rtl',
    }}>

      {/* ── Col 1 (right in RTL): Hamburger on mobile / Nav on desktop ── */}
      {isMobile ? (
        ...
        <nav
          ...
          style={{ display: 'flex', gap: '40px', alignItems: 'center', justifyContent: 'flex-start' }}
        >
          {LINKS.map(link => (
            <NavLink
              key={link.label}
      ...
      {/* ── Col 2 (left in RTL): Logo ── */}
      <div style={{ justifySelf: 'end' }}>
        <img
          src="/Logo.png"
          alt="HighAir Expeditions"
          onClick={() => { navigate('/'); window.scrollTo(0,0); }}
          style={{ height: isMobile ? '60px' : '64px', width: 'auto', display: 'block', objectFit: 'contain', cursor: 'pointer' }}
        />
      </div>
